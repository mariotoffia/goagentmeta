...
// ".cursor/rules/go-rule.mdc" into a flat filename like
// ".cursor__rules__go-rule.mdc" (replacing "/" with "__").
...
func TestGolden_FullProject(t *testing.T) {
	...
	graph := loweredGraph(map[string]pipeline.LoweredObject{
		...
		"go-rule":          keptRule("go-rule", "Use gofmt and golangci-lint.", []string{"**/*.go"}, "Go coding standards"),
