...
func TestIntegrationSimGeneratedHeaderPresent(t *testing.T) {
	...
	for path, content := range pr.MemFS.Files() {
		if strings.HasSuffix(path, ".md") {
			if !strings.Contains(string(content), "generated by goagentmeta") {
				t.Errorf("file %s missing generated header", path)
			}
		}
	}
}
...
func TestIntegrationSimGeneratedHeaderContainsSourceID(t *testing.T) {
	...
	for path, content := range pr.MemFS.Files() {
		if strings.HasSuffix(path, "CLAUDE.md") {
			if !strings.Contains(string(content), "unique-source-id") {
				t.Errorf("CLAUDE.md header should reference source ID, got:\n%s", string(content))
			}
		}
	}
}
