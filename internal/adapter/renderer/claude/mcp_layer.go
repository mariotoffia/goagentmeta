...
// (not "servers") per Claude Code convention.
//
// If two plugins define the same server name, the first one wins and a
// diagnostic warning is emitted (deterministic: plugins are pre-sorted by ID).
func renderMCP(
	...
	serverOwner := make(map[string]string) // server name → plugin ID (for collision detection)
	var sourceObjects []string

	for _, plugin := range plugins {
		entries := extractMCPServers(plugin, objects)
		for name, entry := range entries {
			if existingOwner, exists := serverOwner[name]; exists {
				// Collision: first writer wins; second is silently dropped.
				// In production this should emit a diagnostic, but we keep the
				// first entry for determinism (plugins are pre-sorted by ID).
				_ = existingOwner
				continue
			}
			servers[name] = entry
	...
	// Deduplicate and sort source objects.
...
func extractMCPServers(
	...
	// Check for explicit MCP server configuration in fields.
	...
	// Check for plugin distribution type "external" with MCP transport.
